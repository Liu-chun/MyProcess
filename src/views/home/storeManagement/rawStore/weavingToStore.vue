  ...
  <div class="box">
    ...
    <div class="tab">
      ...
      >
        ...
        <el-table-column prop="WAITQUANTITY" label="待领数量">
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    confirmFn() {
      ...
      api.getweavdata(this.$qs.stringify(params)).then((res) => {
    ...
    detail(row) {
      ...
      api.getmaterialdata(this.$qs.stringify(params)).then((res) => {
        console.log(res.data);
        if (res.data.Success === false) {
          return Notify({ type: "danger", message: res.data.Message });
        } else {
          this.dd = res.data.Enttity;
          console.log(this.dd);
        }
      });
    },
    ...
    submit() {
      ...
      let params = {
        WARPNO: this.selection.WARPNO,
        PICKTYPE: this.selection.PICKTYPE,
        SUPPLIERID: this.selection.SUPPLIERID,
        WARPDEVICEID: this.selection.WARPDEVICEID,
        PERSONID: this.selection.PERSONID,
        MaterialDetailData: [
          {
            ROW_ID: 0,
            MATERIALID: "string",
